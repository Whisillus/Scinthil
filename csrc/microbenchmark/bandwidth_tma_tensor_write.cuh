...
[[nodiscard]] inline bool run_bandwidth_tma_tensor_write(const RunMicrobenchmarkOptions& options,
  ...
  if (!SCINTHIL_CUDA_CHECK(cudaFuncSetAttribute(bandwidth_tma_tensor_write_kernel,
                                                cudaFuncAttributePreferredSharedMemoryCarveout,
                                                cudaSharedmemCarveoutMaxShared))) {
    return false;
  }
  if (dynamic_shared_bytes > properties.sharedMemPerBlock &&
      !SCINTHIL_CUDA_CHECK(cudaFuncSetAttribute(bandwidth_tma_tensor_write_kernel,
  ...
  if (!measure(
          ...
          [=, &resources, &tensor_maps](std::size_t launch_index) {
            ...
            bandwidth_tma_tensor_write_kernel<<<blocks, threads, dynamic_shared_bytes, resources.stream>>>(
  ...
  print_tma_bandwidth_benchmark_result("TMA tensor write-only", options, properties, resources.workspace_count,
                                       elapsed_ms, 1.0);
  return true;
}
