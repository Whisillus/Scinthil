...
__global__ void bandwidth_tma_tensor_read_kernel(const __grid_constant__ CUtensorMap tensor_map, std::size_t tile_count,
  ...
  for (std::size_t tile_index = blockIdx.x; tile_index < tile_count; tile_index += gridDim.x) {
    if (ptx::elect_one_sync()) {
      ...
      ptx::tma_tensor_global_to_shared(&tensor_map, 0, static_cast<int>(tile_index) * coordinate_rows, shared_storage,
                                       barrier);
    }
...
[[nodiscard]] inline bool run_bandwidth_tma_tensor_read(const RunMicrobenchmarkOptions& options,
  ...
  if (!SCINTHIL_CUDA_CHECK(cudaFuncSetAttribute(bandwidth_tma_tensor_read_kernel,
                                                cudaFuncAttributePreferredSharedMemoryCarveout,
                                                cudaSharedmemCarveoutMaxShared))) {
    return false;
  }
  if (dynamic_shared_bytes > properties.sharedMemPerBlock &&
      !SCINTHIL_CUDA_CHECK(cudaFuncSetAttribute(bandwidth_tma_tensor_read_kernel,
  ...
  if (!measure(
          ...
          [=, &resources, &tensor_maps](std::size_t launch_index) {
            ...
            bandwidth_tma_tensor_read_kernel<<<blocks, threads, dynamic_shared_bytes, resources.stream>>>(
  ...
  print_tma_bandwidth_benchmark_result("TMA tensor read-only", options, properties, resources.workspace_count,
                                       elapsed_ms, 1.0);
  return true;
}
