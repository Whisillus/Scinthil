...
[[nodiscard]] inline bool run_bandwidth_global_memory_read_write(const RunMicrobenchmarkOptions& options,
  ...
  auto* input = static_cast<uint4*>(resources->input);
  ...
  if (!SCINTHIL_CUDA_CHECK(cudaOccupancyMaxPotentialBlockSize(
          &minimum_num_blocks, &num_threads_x, bandwidth_global_memory_read_write_kernel<uint4>, 0, 0))) {
    return false;
  }
  if (minimum_num_blocks <= 0 || num_threads_x <= 0) {
    std::fprintf(stderr, "could not determine a valid global-memory read-write launch size\n");
    return false;
  }
  ...
  if (!measure(
          options, resources,
          [=] {
            bandwidth_global_memory_read_write_kernel<uint4>
                <<<blocks, threads, 0, resources->stream>>>(input, output, element_count);
  ...
  print_bandwidth_benchmark_result("global-memory read+write", options, properties, elapsed_ms, 2.0);
  return true;
}
