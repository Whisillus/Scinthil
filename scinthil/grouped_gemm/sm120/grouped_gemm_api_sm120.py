...
def m_grouped_masked_gemm_fp8_sm120(
    ...
    assert cute.rank(mA) == cute.rank(mB) == cute.rank(mD) == 3
    assert cute.rank(scale_a) == cute.rank(scale_b) == 3
    assert cute.rank(mInfo) == 1
    ...
    assert scale_a.element_type == scale_b.element_type == cutlass.Float32
    ...
    assert scale_a.shape == (groups, max_m, gemm_k // recipe_a[1])
    assert scale_b.shape == (groups, n // recipe_b[0], gemm_k // recipe_b[1])
    assert mA.stride[2] == 1 and mB.stride[2] == 1 and mD.stride[2] == 1
    assert scale_a.stride[2] == 1 and scale_b.stride[2] == 1

    gemm = MGroupedMaskedGEMMFP8SM120(
        dtype_a=mA.element_type,
        dtype_b=mB.element_type,
        dtype_d=mD.element_type,
        scale_dtype=scale_a.element_type,
        recipe_a=recipe_a,
        recipe_b=recipe_b,
        tile_m=tile_m,
        tile_n=tile_n,
    ...
    gemm(mA, scale_a, mB, scale_b, mD, mInfo, stream)
