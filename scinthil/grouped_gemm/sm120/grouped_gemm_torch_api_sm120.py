...
from .grouped_gemm_api_sm120 import m_grouped_masked_gemm_sm120
...
def m_grouped_masked_gemm_torch_sm120(
    ...
    *,
    ...
    if masked_m.ndim != 1:
        raise ValueError("masked_m must be a rank-1 tensor")
    if not a.is_cuda or not b.is_cuda or not masked_m.is_cuda:
        raise ValueError("a, b, and masked_m must be CUDA tensors")
    if a.device != b.device or a.device != masked_m.device:
        raise ValueError("a, b, and masked_m must be on the same CUDA device")
    ...
    if masked_m.dtype != torch.int32:
        raise TypeError("masked_m must have dtype torch.int32")
    if not a.is_contiguous() or not b.is_contiguous() or not masked_m.is_contiguous():
        raise ValueError("a, b, and masked_m must be contiguous")
    if a.data_ptr() % 16 != 0 or b.data_ptr() % 16 != 0 or masked_m.data_ptr() % 16 != 0:
        raise ValueError("a, b, and masked_m must be 16-byte aligned")

    groups, max_m, gemm_k = a.shape
    groups_b, gemm_n, gemm_k_b = b.shape
    if groups_b != groups:
        raise ValueError("a and b must have the same group count")
    if gemm_k_b != gemm_k:
        raise ValueError("a and b must have the same K dimension")
    if masked_m.shape != (groups,):
        raise ValueError("masked_m must have shape [G]")
    if min(groups, max_m, gemm_n, gemm_k) <= 0:
    ...
    if out is None:
        out = torch.empty((groups, max_m, gemm_n), dtype=a.dtype, device=a.device)
    else:
        if out.ndim != 3:
        ...
        if out.shape != (groups, max_m, gemm_n):
    ...
    with torch.cuda.device(a.device):
        ...
        m_grouped_masked_gemm_sm120(
            ...
            from_dlpack(masked_m.detach(), assumed_align=16),
            stream,
        )
    return out
